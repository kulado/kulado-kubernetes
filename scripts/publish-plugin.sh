...
PLUGIN_PACKAGE_NAME="kulado-resource-kubernetes-${VERSION}-$(go env GOOS)-$(go env GOARCH).tar.gz"
...
go build \
   -ldflags "-X github.com/kulado/kulado-kubernetes/pkg/version.Version=${VERSION}" \
   -o "${WORK_PATH}/kulado-resource-kubernetes${BIN_SUFFIX}" \
   "${ROOT}/cmd/kulado-resource-kubernetes"
...
CREDS_JSON=$(aws sts assume-role \
                 ...
                 --role-session-name "upload-plugin-kulado-resource-kubernetes" \
