...
// SkipAwaitLogic returns true if the `kulado.com/skipAwait` annotation is "true", false otherwise.
func SkipAwaitLogic(obj *unstructured.Unstructured) bool {
	return IsAnnotationTrue(obj, AnnotationSkipAwait)
}

// TimeoutSeconds returns the int value of the `kulado.com/timeoutSeconds` annotation, or the defaultSeconds value
