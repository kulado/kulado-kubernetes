...
import (
	...
	"github.com/kulado/kulado-kubernetes/pkg/await"
	"github.com/kulado/kulado-kubernetes/pkg/clients"
	"github.com/kulado/kulado-kubernetes/pkg/logging"
	"github.com/kulado/kulado-kubernetes/pkg/metadata"
	"github.com/kulado/kulado-kubernetes/pkg/openapi"
	"github.com/kulado/kulado/pkg/resource"
	"github.com/kulado/kulado/pkg/resource/plugin"
	"github.com/kulado/kulado/pkg/resource/provider"
	"github.com/kulado/kulado/pkg/util/contract"
	"github.com/kulado/kulado/pkg/util/rpcutil/rpcerror"
	kuladorpc "github.com/kulado/kulado/sdk/proto/go"
...
//
// Implements functionality for the Kulado Kubernetes Resource Provider. This code is responsible
...
var _ kuladorpc.ResourceProviderServer = (*kubeProvider)(nil)

func makeKubeProvider(
	host *provider.HostClient, name, version string,
) (kuladorpc.ResourceProviderServer, error) {
	return &kubeProvider{
		host:            host,
...
func (k *kubeProvider) CheckConfig(ctx context.Context, req *kuladorpc.CheckRequest) (*kuladorpc.CheckResponse, error) {
	return &kuladorpc.CheckResponse{Inputs: req.GetNews()}, nil
}

// DiffConfig diffs the configuration for this provider.
func (k *kubeProvider) DiffConfig(ctx context.Context, req *kuladorpc.DiffRequest) (*kuladorpc.DiffResponse, error) {
	...
	if news["kubeconfig"].IsComputed() {
		return &kuladorpc.DiffResponse{
			Changes:  kuladorpc.DiffResponse_DIFF_SOME,
	...
	return &kuladorpc.DiffResponse{
		Changes: kuladorpc.DiffResponse_DIFF_NONE,
	}, nil
}

// Configure configures the resource provider with "globals" that control its behavior.
func (k *kubeProvider) Configure(_ context.Context, req *kuladorpc.ConfigureRequest) (*kuladorpc.ConfigureResponse, error) {
	...
	return &kuladorpc.ConfigureResponse{}, nil
}

// Invoke dynamically executes a built-in function in the provider.
func (k *kubeProvider) Invoke(ctx context.Context,
	req *kuladorpc.InvokeRequest) (*kuladorpc.InvokeResponse, error) {
	...
	case invokeKubectlReplace:
		...
		return &kuladorpc.InvokeResponse{Return: objProps}, nil
	default:
...
func (k *kubeProvider) Check(ctx context.Context, req *kuladorpc.CheckRequest) (*kuladorpc.CheckResponse, error) {
	//
	...
	var failures []*kuladorpc.CheckFailure
	...
	for k := range newInputs.GetAnnotations() {
		if metadata.IsInternalAnnotation(k) {
			failures = append(failures, &kuladorpc.CheckFailure{
	...
	if len(oldInputs.Object) > 0 {
		...
		// If this resource does not have a "managed-by: kulado" label in its inputs, it is likely we are importing
		...
		// Set a "managed-by: kulado" label on all created k8s resources.
		_, err = metadata.TrySetManagedByLabel(newInputs)
		if err != nil {
			return nil, pkgerrors.Wrapf(err,
				"Failed to create object because of a problem setting managed-by labels")
	...
	return &kuladorpc.CheckResponse{Inputs: autonamedInputs, Failures: failures}, nil
}

// Diff checks what impacts a hypothetical update will have on the resource's properties.
func (k *kubeProvider) Diff(
	ctx context.Context, req *kuladorpc.DiffRequest,
) (*kuladorpc.DiffResponse, error) {
	//
	...
	hasChanges := kuladorpc.DiffResponse_DIFF_NONE

	var changes, replaces []string
	var detailedDiff map[string]*kuladorpc.PropertyDiff
	if len(patchObj) != 0 {
		hasChanges = kuladorpc.DiffResponse_DIFF_SOME
		...
		for k, v := range detailedDiff {
			switch v.Kind {
			case kuladorpc.PropertyDiff_ADD_REPLACE, kuladorpc.PropertyDiff_DELETE_REPLACE, kuladorpc.PropertyDiff_UPDATE_REPLACE:
...
func (k *kubeProvider) Create(
	ctx context.Context, req *kuladorpc.CreateRequest,
) (*kuladorpc.CreateResponse, error) {
	//
	...
	return &kuladorpc.CreateResponse{
...
func (k *kubeProvider) Read(ctx context.Context, req *kuladorpc.ReadRequest) (*kuladorpc.ReadResponse, error) {
	//
	// Behavior as of v0.12.x: We take 1 input:
	//
	...
	return &kuladorpc.ReadResponse{Id: id, Properties: state, Inputs: inputs}, nil
}
...
func (k *kubeProvider) Update(
	ctx context.Context, req *kuladorpc.UpdateRequest,
) (*kuladorpc.UpdateResponse, error) {
	//
	// Behavior as of v0.12.x: We take 2 inputs:
	//
	// 1. req.News, the new resource inputs, i.e., the property bag coming from a custom resource like
	...
	return &kuladorpc.UpdateResponse{Properties: inputsAndComputed}, nil
}

// Delete tears down an existing resource with the given ID.  If it fails, the resource is assumed
// to still exist.
func (k *kubeProvider) Delete(
	ctx context.Context, req *kuladorpc.DeleteRequest,
) (*pbempty.Empty, error) {
	urn := resource.URN(req.GetUrn())
...
func (k *kubeProvider) GetPluginInfo(context.Context, *pbempty.Empty) (*kuladorpc.PluginInfo, error) {
	return &kuladorpc.PluginInfo{
		Version: k.version,
	}, nil
}
...
func parseCheckpointObject(obj resource.PropertyMap) (oldInputs, live *unstructured.Unstructured) {
	...
	//
	// [1]: https://github.com/kulado/kulado-kubernetes/issues/137
	//
...
func partialError(id string, err error, state *structpb.Struct, inputs *structpb.Struct) error {
	...
	detail := kuladorpc.ErrorResourceInitFailed{
		Id:         id,
		Properties: state,
		Reasons:    reasons,
		Inputs:     inputs,
...
var deleteResponse = &kuladorpc.ReadResponse{Id: "", Properties: nil}
...
func parseLiveInputs(live, oldInputs *unstructured.Unstructured) *unstructured.Unstructured {
	...
	// attempt to propagate the live object's GVK, any Kulado-generated autoname and its annotation, and return
	// the result.
...
// convertPatchToDiff converts the given JSON merge patch to a Kulado detailed diff.
func convertPatchToDiff(
	patch, oldLiveState, newInputs, oldInputs map[string]interface{}, gvk schema.GroupVersionKind,
) (map[string]*kuladorpc.PropertyDiff, error) {
	...
	pc := &patchConverter{
		...
		diff:     map[string]*kuladorpc.PropertyDiff{},
	}
...
func (pc *patchConverter) addPatchValueToDiff(
	...
	var diffKind kuladorpc.PropertyDiff_Kind
	inputDiff := false
	if v == nil {
		diffKind, inputDiff = kuladorpc.PropertyDiff_DELETE, true
	} else if old == nil {
		diffKind = kuladorpc.PropertyDiff_ADD
	} else {
		switch v := v.(type) {
		case map[string]interface{}:
			...
			diffKind = kuladorpc.PropertyDiff_UPDATE
		case []interface{}:
	...
	if len(matches) != 0 {
		switch diffKind {
		case kuladorpc.PropertyDiff_ADD:
			diffKind = kuladorpc.PropertyDiff_ADD_REPLACE
		case kuladorpc.PropertyDiff_DELETE:
			diffKind = kuladorpc.PropertyDiff_DELETE_REPLACE
		case kuladorpc.PropertyDiff_UPDATE:
			diffKind = kuladorpc.PropertyDiff_UPDATE_REPLACE
		}
	}
	...
	pc.diff[pathStr] = &kuladorpc.PropertyDiff{Kind: diffKind, InputDiff: inputDiff}
	return nil
}
